...
public class LatHinh extends LangLa_fo {
    ...
    public Item[] arrayI = new Item[30];
    public BoxItem[] arrayBoxItem = new BoxItem[30];
    public int c2 = 0;
    ...
    public LatHinh(MainScreen var1) {
        ...
        this.r = false;
        this.cF = 2;
        btn_TuLat = this.a((this.width / 2) - 30 - 45, getZ(this.height, 90), "Lật Hình", this, 9999, -100);
        btn_ThayDoi = this.a((this.width / 2) - 30 +10, getZ(this.height, 90), "Thay đổi", this, 10000, -100);
        btn_ThuLai = this.a((this.width / 2) - 30 + 65, getZ(this.height, 90), "Thử Lại", this, 10001, -100);
        btn_ThuLai.a(true);
        btn_ThayDoi.a(false);
    }
    ...
    public void a(Graphics var1) {
        ...
        for (int i = 0; i < arrayI.length; i++) {
            arrayI[i] = new Item(774);
            arrayI[i].level = 0;
            arrayI[i].rangeNE = true;
        }
        int cols = 6;   // số cột
        int rows = 5;   // số hàng
        int size = 35;  // kích thước ô
        int spacing = 5; // khoảng cách giữa các ô

        // chiều rộng tổng của lưới
        int totalWidth = cols * (size + spacing) - spacing;
        // chiều cao tổng của lưới
        int totalHeight = rows * (size + spacing) - spacing;

        // căn giữa theo panel
        int startX = 5 + (this.width - totalWidth) / 2;
        int startY = 37; // cách tiêu đề 1 đoạn
...
//                Binary2.a(var1, -5, 0, box.xMain + 2, box.yMain + 2, 25, (int) 25);
    ...
    public Vector c() {
        ...
        for (int i = 0; i < arrayBoxItem.length; i++) {
            if (arrayBoxItem[i] != null) {
                vec.add(new LangLa_jz(2907 + i,
                    arrayBoxItem[i].xMain,
                    arrayBoxItem[i].yMain,
                    arrayBoxItem[i].xMain + 35,
                    arrayBoxItem[i].yMain + 35,
                    null, this));
            }
        }
        return vec;
